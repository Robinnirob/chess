...
class GameManager:
    ...
    def manage_pawn_moves(self, position: Position = None) -> List[Move]:
        ...
        result = []
        self.add_if_offset_position_has_no_piece(position=my_position, result=result, row=direction)
        if is_initial_position: self.add_if_offset_position_has_no_piece(position=my_position, result=result, row=direction * 2, is_two_step_pawn_move=True)
        self.add_if_offset_position_has_opponent_piece(position=my_position, result=result, row=direction, col=1)
        self.add_if_offset_position_has_opponent_piece(position=my_position, result=result, row=direction, col=-1)
    ...
    def manage_knight_moves(self, position: Position = None) -> List[Move]:
        my_position = position if position is not None else self.selected_position
        result = []
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=-1, col=-2)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=1, col=-2)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=-2, col=-1)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=2, col=-1)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=-2, col=1)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=2, col=1)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=-1, col=2)
    ...
    def manage_rook_moves(self, position: Position = None) -> List[Move]:
        my_position = position if position is not None else self.selected_position
        result = []
    ...
    def manage_rook_threat(self, position: Position = None) -> List[Position]:
        result = []
        for row_offset in range(1, 8):
            ...
            if self.board.get_piece(offset_position) is not None:
                break
        for row_offset in range(-1, -8, -1):
            offset_position = position.offset(row=row_offset)
            if not offset_position.belong_to_board():
    ...
    def manage_bishop_moves(self, position: Position = None) -> List[Move]:
        my_position = position if position is not None else self.selected_position
        result = []
    ...
    def manage_bishop_threat(self, position: Position = None) -> List[Position]:
        result = []
    ...
    def manage_queen_moves(self, position: Position = None) -> List[Move]:
        my_position = position if position is not None else self.selected_position
        result = []
        result.extend(self.manage_bishop_moves(position=my_position))
        result.extend(self.manage_rook_moves(position=my_position))
        return result

    def manage_queen_threat(self, position: Position = None) -> List[Position]:
        result = []
        result.extend(self.manage_bishop_threat(position=position))
        result.extend(self.manage_rook_threat(position=position))
        return result

    def manage_king_moves(self, position: Position = None) -> List[Move]:
        my_position = position if position is not None else self.selected_position
        selected_piece = self.board.get_piece(my_position)
        result = []
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=1, col=0)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=-1, col=0)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=0, col=1)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=0, col=-1)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=1, col=1)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=1, col=-1)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=-1, col=1)
        self.add_if_offset_position_has_not_current_color_piece(position=my_position, result=result, row=-1, col=-1)
        _, filtered_position = self.filter_position_threated(positions=[move.target for move in result], color=selected_piece.color.opposite_color())
    ...
    def add_if_offset_position_has_no_piece(self, position: Position, result: List[Move], row: int = 0, col: int = 0, is_two_step_pawn_move: bool = False) -> bool:
        if (self.is_offset_belong_to_board(position=position, row=row, col=col) and
                not self.is_offset_position_has_piece_between(position=position, row=row, col=col)):
        ...
        return (self.is_offset_position_has_piece(position=position, row=row, col=col) or
                not self.is_offset_belong_to_board(position=position, row=row, col=col))

    def add_if_offset_position_has_not_current_color_piece(self, position: Position, result: List[Move], row: int = 0, col: int = 0, is_two_step_pawn_move: bool = False) -> bool:
        if (self.is_offset_belong_to_board(position=position, row=row, col=col) and
                self.is_offset_belong_to_board(position=position, row=row, col=col) and
                not self.is_offset_position_has_current_color_piece(position=position, row=row, col=col)):
            target_position = position.offset(row=row, col=col)
    ...
    def add_if_offset_position_has_opponent_piece(self, position: Position, result: List[Move], row: int = 0, col: int = 0, is_two_step_pawn_move: bool = False) -> bool:
        if (self.is_offset_belong_to_board(position=position, row=row, col=col) and
                self.is_offset_position_has_opponent_piece(position=position, row=row, col=col)):
            target_position = position.offset(row=row, col=col)
            result.append(self.move_factory(target_position, self.board.get_piece(target_position), is_two_step_pawn_move=is_two_step_pawn_move))
        return (self.is_offset_position_has_piece(position=position, row=row, col=col) or
                not self.is_offset_belong_to_board(position=position, row=row, col=col))

    def is_offset_position_has_piece_between(self, position: Position, row: int = 0, col: int = 0) -> bool:
        if row != 0:
            step = 1 if row > 0 else -1
            for i in range(0, row, step):
                if self.board.get_piece(position.offset(row=i + step, col=col)) is not None: return True
        if col != 0:
            step = 1 if col > 0 else -1
            for i in range(0, col, step):
                if self.board.get_piece(position.offset(row=row, col=i + step)) is not None: return True
        return False

    def is_offset_position_has_selected_color_piece(self, position: Position, row: int = 0, col: int = 0) -> bool:
        piece = self.board.get_piece(position)
        piece_on_offset_position = self.board.get_piece(position.offset(row=row, col=col))
        return piece_on_offset_position is not None and piece_on_offset_position.color == piece.color

    def is_offset_position_has_opponent_piece(self, position: Position, row: int = 0, col: int = 0) -> bool:
        piece = self.board.get_piece(position)
        piece_on_offset_position = self.board.get_piece(position.offset(row=row, col=col))
        return piece_on_offset_position is not None and piece_on_offset_position.color != piece.color

    def is_offset_position_has_current_color_piece(self, position: Position, row: int = 0, col: int = 0) -> bool:
        piece = self.board.get_piece(position)
        piece_on_offset_position = self.board.get_piece(position.offset(row=row, col=col))
        return piece_on_offset_position is not None and piece_on_offset_position.color == piece.color

    def is_offset_position_has_piece(self, position: Position, row: int = 0, col: int = 0) -> bool:
        piece_on_offset_position = self.board.get_piece(position.offset(row=row, col=col))
    ...
    def filter_position_threated(self, positions: List[Position], color: PieceColor) -> tuple[Set[Position], Set[Position]]:
        final_threated_positions = set()
        final_unthreated_positions = set(positions)
        pieces_position = self.board.get_pieces_position(color)
        for position, piece in pieces_position.items():
            ...
            if piece.name == PieceName.PAWN:
                ...
                threat_positions = self.manage_rook_threat(position)
            elif piece.name == PieceName.BISHOP:
                threat_positions = self.manage_bishop_threat(position)
            elif piece.name == PieceName.QUEEN:
                threat_positions = self.manage_queen_threat(position)
